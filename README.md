...
4. If you would like to also build the ModDeployer plugin, additional steps are required. The required steps are provided below. Otherwise, skip to step 5.
    ...
    - Navigate to the folder where the .tar.gz was downloaded in a terminal window and execute the following commands:
        ...
        This will extract libc++ binaries from the .tar.xz file into the `~/.llvm-libcpp-11.0.1` directory. If you would like, you can change NEW_LIBCPP_PATH to point to a different directory, but you will have to modify the `NEW_LIBCPP_PATH` variable in the `Plugins/ModDeployer/Source/ModDeployer/ModDeployer.Build.cs` file to reflect the change.
    - Skip to step 6.
